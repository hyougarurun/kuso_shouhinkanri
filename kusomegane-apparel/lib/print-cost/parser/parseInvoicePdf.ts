...
import type { InvoiceLineItem, ParsedInvoice } from "../types"
import { normalizeLocation } from "../normalizer/location"
import { normalizeMethod } from "../normalizer/method"

const PARSER_VERSION = "kusomegane-apparel-g2-0.1.0"
...
export async function parseInvoicePdf(
  ...
  if (!block || block.type !== "text") {
    throw new Error(
      `Claude が text ブロックを返しませんでした (stop_reason=${response.stop_reason})`
    )
  }
  ...
  try {
    ...
    throw new Error(
      `JSON parse failed (stop_reason=${response.stop_reason}, length=${jsonText.length}). ` +
